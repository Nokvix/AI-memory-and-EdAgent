...
from app.services.ai_letter_generation import generate_letter_with_gigachat
...
def generate_letter_content(
    ...
    """
    Сгенерировать письмо (тему и тело).
    Использует GigaChat для генерации тела письма.

    Args:
        company_name: Название компании, для которой формируется письмо.
        skills: Список технологий/навыков (стек), который будет упомянут в письме.
        template: Тип шаблона письма. Допустимые значения: "formal" или "informal".
        contact_email: Контактный email для обратной связи. Будет добавлен в письмо,
            если его нет в сгенерированном тексте.
        university_name: Название университета/организации, от имени которой
            отправляется письмо (используется в теме письма).

    Returns:
        Словарь с результатом генерации:
        - "subject": Тема письма (str).
        - "body": Тело письма (str).
        - "template": Использованный шаблон (str).

    Raises:
        ValueError: Если передан недопустимый тип шаблона (не "formal" и не "informal").
    """
    if template not in ["formal", "informal"]:
        raise ValueError(f"Недопустимый тип шаблона: {template}")

    if template == "formal":
        subject = f"Партнёрство с {company_name} — предложение от {university_name}"
    else:
        subject = f"Сотрудничество с {company_name} и ПроКомпетенции УрФУ 🚀"

    body = generate_letter_with_gigachat(company_name, skills, template)

    if contact_email not in body:
        body += f"\n\nКонтакты для связи: {contact_email}"
